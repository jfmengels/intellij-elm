...
class ElmExternalReviewActionTest : ElmWorkspaceTestBase() {

    override fun runTest() {
        if (toolchain.elmReviewCLI == null) {
            System.err.println("SKIP $name: elm-review not found")
            return
        }
        super.runTest()
    }
    ...
    private fun makeTestAction(file: VirtualFile): Pair<ElmExternalReviewAction, TestActionEvent> {
        val dataContext = MapDataContext(mapOf(
                CommonDataKeys.PROJECT to project,
                CommonDataKeys.VIRTUAL_FILE to file
        ))
        val action = ElmExternalReviewAction()
